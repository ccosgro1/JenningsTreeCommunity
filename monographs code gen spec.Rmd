...
Let's classify things as generalists v specialists statistically.
```{r indval}
##Load the data and subset to just the cores in BUDR
sppabun=read.csv("C:/Users/crc31/OneDrive/Documents/School/Research/Data/Trees/Adult Plot Species Matrix.csv")
cores=sppabun[sppabun$Transect=="Core",]
cores=cores[cores$Ecosystem!="S",]
##Remove species that don't have individuals in the core (these are: ACESIL, CARTOM, CORFLOR, CRAPUN, MALCOR). Also remove Transect and TerrType because those shouldn't be necessary anymore and we're already removing stuff.
colnames(cores)
cores=cores[,c(1:2,5:6,8:11,15:20,22:37)]

indvalsummary=indval(cores[,3:30], clustering=cores$Ecosystem)
summary(indvalsummary, p=0.05,type='short', digits=2, show=p, sort=FALSE)
#This would suggest that FRANIG, QUEPAL, ULMSP, LIRTUP, PLAOCC, ACESAC, FAGAME, and PRUSER are all specialists with reference to the core ecosystems.


###Let's try this again with the entire forest. For shits and giggs.
indvaltotal=indval(sppabun[,5:37],clustering=sppabun$Ecosystem)
summary(indvaltotal, p=0.05,type='short', digits=2, show=p, sort=FALSE)
#This suggests that, relative to the entire ecosystem, only CARCAR, TILAME, and PLAOCC are specialists.


###One last time with reference to edges.
#subset the cores out.
edges=sppabun[sppabun$Transect=="Edge"|sppabun$Transect=="Trans",]
#remove species that aren't present (ACESIL, CARTOM, QUEBIC, SALNIG, TOXVER, VIBLEN)
edges=edges[,c(1:2,5:6,8:11,13:28,30:31,33:34,36)]
#run the stats
indvalsummary=indval(edges[,3:29], clustering=edges$Ecosystem)
summary(indvalsummary, p=0.05,type='short', digits=2, show=p, sort=FALSE)
...
#generalists
tree.hel.soil.gen=tree.hel.soil[,c(1:10,15:16,27:28,33,36,46:79)]

### Generalists ###
# Here is the global analysis. Can proceed if significant.  Use adj. R square as additional stopping criterion.
hel.rda.fulsoil = rda(tree.hel.soil.gen[,10:16], tree.hel.soil.gen[,26:50])
...
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,10:16], tree.hel.all.pcnm[,51:93])
...
varpart(tree.hel.all.pcnm[,10:16], tree.hel.all.pcnm[,c(35,37,39,41)], tree.hel.all.pcnm[,c(60:61,65:68)])
...
# Let's see.
hel.rda.all.partpcnm=rda(tree.hel.all.pcnm[,10:16], tree.hel.all.pcnm[,c(30,43)])
anova(hel.rda.all.partpcnm)
# Yes.

colnames(tree.hel.all.pcnm)
varpart(tree.hel.all.pcnm[,10:16], tree.hel.all.pcnm[,c(30,43)])
...
hel.rda.selsoil = rda(tree.hel.soil.gen[,10:16] ~ as.matrix(tree.hel.soil.gen[,c(26,49,44,28)]))
anova(hel.rda.selsoil)
RsquareAdj(hel.rda.selsoil)
plot(hel.rda.selsoil)
# Cool analysis, but what a terrible plot.  Much better to get the scores and plot them in a real graphing program, or do some fancy graphing in R if that is your thing.
...
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,10:16], tree.hel.all.pcnm[,51:75])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared
# now run step 2, the selection part
hel.rda.fulsoil.none = rda(tree.hel.all.pcnm[,10:16]~1)
hel.rda.fulsoil = rda(tree.hel.all.pcnm[,10:16], tree.hel.all.pcnm[,51:75])

hel.rda.fulsoil.full=rda(tree.hel.all.pcnm[,10:16]~tree.hel.all.pcnm$PCNM1+tree.hel.all.pcnm$PCNM2+tree.hel.all.pcnm$PCNM3+tree.hel.all.pcnm$PCNM4+tree.hel.all.pcnm$PCNM5+tree.hel.all.pcnm$PCNM6+tree.hel.all.pcnm$PCNM7+tree.hel.all.pcnm$PCNM8+tree.hel.all.pcnm$PCNM9+tree.hel.all.pcnm$PCNM10+tree.hel.all.pcnm$PCNM11+tree.hel.all.pcnm$PCNM12+tree.hel.all.pcnm$PCNM13+tree.hel.all.pcnm$PCNM14+tree.hel.all.pcnm$PCNM15+tree.hel.all.pcnm$PCNM16+tree.hel.all.pcnm$PCNM17+tree.hel.all.pcnm$PCNM18+tree.hel.all.pcnm$PCNM19+tree.hel.all.pcnm$PCNM20+tree.hel.all.pcnm$PCNM21+tree.hel.all.pcnm$PCNM22+tree.hel.all.pcnm$PCNM23+tree.hel.all.pcnm$PCNM24+tree.hel.all.pcnm$PCNM25)

tree.sel=ordiR2step(hel.rda.fulsoil.none,scope=formula(hel.rda.fulsoil.full),Pin=0.05,R2scope=TRUE)
tree.sel
# Let's store a model using the selected variables only
names(tree.hel.all.pcnm)
hel.rda.all.selpcnm = rda(tree.hel.all.pcnm[,10:16] ~ as.matrix(tree.hel.all.pcnm[,c(51:55,57:60,63,65)]))
anova(hel.rda.all.selpcnm)
RsquareAdj(hel.rda.all.selpcnm)

# Now that we have these parsimonious models, NOW WE CAN PERFORM VARIANCE PARTITIONING!  Hurray!
colnames(tree.hel.all.pcnm)
varpart(tree.hel.all.pcnm[,10:16], tree.hel.all.pcnm[,c(26,49,44,28)], tree.hel.all.pcnm[,c(51:55,57:60,63,65)])
# The variance explained by soil variables and spatial structure clearly overlap. 
# It also appears that there is more independent spatial structure in the trees than independent structuring by soil.
# Are the soil variables significant after accounting for spatial structure, and vice versa?
# Let's see.
hel.rda.all.partpcnm=rda(tree.hel.all.pcnm[,10:16], tree.hel.all.pcnm[,c(51:55,57:60,63,65)], tree.hel.all.pcnm[,c(26,49,44,28)])
anova(hel.rda.all.partpcnm)
# NO.
hel.rda.all.partsoil=rda(tree.hel.all.pcnm[,10:43], tree.hel.all.pcnm[,c(26,49,44,28)], tree.hel.all.pcnm[,c(51:55,57:60,63,65)])
...
names(tree.hel.soil.gen)
hel.rda.selsoil = rda(tree.hel.soil.gen[,10:16] ~ as.matrix(tree.hel.soil.gen[,c(30,49,28,33,42)]))
anova(hel.rda.selsoil)
RsquareAdj(hel.rda.selsoil)
plot(hel.rda.selsoil)
# Cool analysis, but what a terrible plot.  Much better to get the scores and plot them in a real graphing program, or do some fancy graphing in R if that is your thing.
...
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,10:16], tree.hel.all.pcnm[,51:89])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared
# now run step 2, the selection part
hel.rda.fulsoil.none = rda(tree.hel.all.pcnm[,10:16]~1)
hel.rda.fulsoil = rda(tree.hel.all.pcnm[,10:16], tree.hel.all.pcnm[,51:89])
...
# Let's store a model using the selected variables only
names(tree.hel.all.pcnm)
hel.rda.all.selpcnm = rda(tree.hel.all.pcnm[,10:16] ~ as.matrix(tree.hel.all.pcnm[,c(52:53,58,60)]))
anova(hel.rda.all.selpcnm)
RsquareAdj(hel.rda.all.selpcnm)

# Now that we have these parsimonious models, NOW WE CAN PERFORM VARIANCE PARTITIONING!  Hurray!
colnames(tree.hel.all.pcnm)
varpart(tree.hel.all.pcnm[,10:16], tree.hel.all.pcnm[,c(30,49,28,33,42)], tree.hel.all.pcnm[,c(52:53,58,60)])
# The variance explained by soil variables and spatial structure clearly overlap. 
# It also appears that there is more independent spatial structure in the trees than independent structuring by soil.
# Are the soil variables significant after accounting for spatial structure, and vice versa?
# Let's see.
hel.rda.all.partpcnm=rda(tree.hel.all.pcnm[,10:16], tree.hel.all.pcnm[,c(52:53,58,60)], tree.hel.all.pcnm[,c(30,49,28,33,42)])
anova(hel.rda.all.partpcnm)
# Yes.
hel.rda.all.partsoil=rda(tree.hel.all.pcnm[,10:16], tree.hel.all.pcnm[,c(30,49,28,33,42)], tree.hel.all.pcnm[,c(52:53,58,60)])
anova(hel.rda.all.partsoil)
# Also yes.  But only by p-value; that is really not much variance explained anymore.
...
tree.hel.core.rda.ecosys = rda(tree.hel.core[,10:16] ~ tree.hel.core[,3])
...
tree.hel.soil=tree.hel.soil[,c(1:10,15:16,27:28,33,36,52:76)]

# Here is the global analysis. Can proceed if significant.  Use adj. R square as additional stopping criterion.
hel.rda.fulsoil = rda(tree.hel.soil[,10:16], tree.hel.soil[,17:41])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared
...
names(tree.hel.all.pcnm)
## Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,10:16], tree.hel.all.pcnm[,42:75])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared
...
colnames(tree.hel.soil)
# Here is the global analysis. Can proceed if significant.  Use adj. R square as additional stopping criterion.
hel.rda.fulsoil = rda(tree.hel.soil[,10:16], tree.hel.soil[,10:16])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared
...
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,10:16], tree.hel.all.pcnm[,42:60])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared
...
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,10:16], tree.hel.all.pcnm[,42:69])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared
...
# Then the core+edge plots
tree.hel.edge=tree.hel[tree.hel$EP=="Core"|tree.hel$EP=="Edge",]
tree.hel.edge.rda.ecosys = rda(tree.hel.edge[,10:16] ~ tree.hel.edge[,3])
anova(tree.hel.edge.rda.ecosys)
edge.arsq=RsquareAdj(tree.hel.edge.rda.ecosys)
# Then the permmutation test of the edge effect
edge.eff=core.arsq$adj.r.squared/edge.arsq$adj.r.squared
...
##Same idea as above, but Jaccard distance analysis by PCoA/distance-based RDA
jac.core.dist=vegdist(tree.hel.core[,10:16],method="jaccard",binary=TRUE)
jac.core.pcoa=cmdscale(jac.core.dist,eig=TRUE,k=41,add=TRUE)
jac.core.rda.ecosys = rda(jac.core.pcoa$points ~ tree.hel.core[,3])
anova(jac.core.rda.ecosys)
jac.core.arsq=RsquareAdj(jac.core.rda.ecosys)
jac.edge.dist=vegdist(tree.hel.edge[,10:16],method="jaccard",binary=TRUE)
jac.edge.pcoa=cmdscale(jac.edge.dist,eig=TRUE,k=70,add=TRUE)
jac.edge.pcoa.points=cbind(jac.edge.pcoa$points,tree.hel.edge)
jac.edge.rda.ecosys = rda(jac.edge.pcoa.points[,1:70] ~ jac.edge.pcoa.points[,73])
anova(jac.edge.rda.ecosys)
...
tree.hel.soil=tree.hel.soil[,c(1:10,14:15,25,27,30,47:71)]

# Here is the global analysis. Can proceed if significant.  Use adj. R square as additional stopping criterion.
hel.rda.fulsoil = rda(tree.hel.soil[,10:15], tree.hel.soil[,16:40])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared
...
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,10:15], tree.hel.all.pcnm[,41:76])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared
...
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,10:15], tree.hel.all.pcnm[,41:61])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared
...
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,10:15], tree.hel.all.pcnm[,41:74])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared
...
#separate out the generalists
tree.hel.soil=tree.hel.soil[,c(1:10,15:16,26:27,32,35,54:78)]
...
#Forward seclection
hel.rda.fulsoil.none = rda(tree.hel.all.pcnm[,10:16]~1)
hel.rda.fulsoil = rda(tree.hel.all.pcnm[,10:16], tree.hel.all.pcnm[,42:75])

hel.rda.fulsoil.full=rda(tree.hel.all.pcnm[,10:16]~tree.hel.all.pcnm$PCNM1+tree.hel.all.pcnm$PCNM2+tree.hel.all.pcnm$PCNM3+tree.hel.all.pcnm$PCNM4+tree.hel.all.pcnm$PCNM5+tree.hel.all.pcnm$PCNM6+tree.hel.all.pcnm$PCNM7+tree.hel.all.pcnm$PCNM8+tree.hel.all.pcnm$PCNM9+tree.hel.all.pcnm$PCNM10+tree.hel.all.pcnm$PCNM11+tree.hel.all.pcnm$PCNM12+tree.hel.all.pcnm$PCNM13+tree.hel.all.pcnm$PCNM14+tree.hel.all.pcnm$PCNM15+tree.hel.all.pcnm$PCNM16+tree.hel.all.pcnm$PCNM17+tree.hel.all.pcnm$PCNM18+tree.hel.all.pcnm$PCNM19+tree.hel.all.pcnm$PCNM20+tree.hel.all.pcnm$PCNM21+tree.hel.all.pcnm$PCNM22+tree.hel.all.pcnm$PCNM23+tree.hel.all.pcnm$PCNM24+tree.hel.all.pcnm$PCNM25+tree.hel.all.pcnm$PCNM26+tree.hel.all.pcnm$PCNM27+tree.hel.all.pcnm$PCNM28+tree.hel.all.pcnm$PCNM29+tree.hel.all.pcnm$PCNM30+tree.hel.all.pcnm$PCNM31+tree.hel.all.pcnm$PCNM32+tree.hel.all.pcnm$PCNM33+tree.hel.all.pcnm$PCNM34)

tree.sel=ordiR2step(hel.rda.fulsoil.none,scope=formula(hel.rda.fulsoil.full),Pin=0.05,R2scope=TRUE)
tree.sel
## Let's store a model using the selected variables only
names(tree.hel.all.pcnm)
hel.rda.all.selpcnm = rda(tree.hel.all.pcnm[,10:16] ~ as.matrix(tree.hel.all.pcnm[,c(47:48,50,52,68)]))
anova(hel.rda.all.selpcnm)
RsquareAdj(hel.rda.all.selpcnm)
...
##NEW forward selection
hel.rda.fulsoil.none = rda(tree.hel.soil.gen[,10:16]~1)
hel.rda.fulsoil = rda(tree.hel.soil.gen[,10:16], tree.hel.soil.gen[,17:41])
...
hel.rda.selsoil = rda(tree.hel.soil.gen[,10:16] ~ as.matrix(tree.hel.soil.gen[,c(21,34)]))
anova(hel.rda.selsoil)
RsquareAdj(hel.rda.selsoil)
plot(hel.rda.selsoil)
# Cool analysis, but what a terrible plot.  Much better to get the scores and plot them in a real graphing program, or do some fancy graphing in R if that is your thing.
...
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,10:16], tree.hel.all.pcnm[,42:59])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

hel.rda.fulsoil.none = rda(tree.hel.all.pcnm[,10:16]~1)
hel.rda.fulsoil = rda(tree.hel.all.pcnm[,10:16], tree.hel.all.pcnm[,42:59])
...
## Let's store a model using the selected variables only
names(tree.hel.all.pcnm)
hel.rda.all.selpcnm = rda(tree.hel.all.pcnm[,10:16] ~ as.matrix(tree.hel.all.pcnm[,c(42,44:45,52)]))
anova(hel.rda.all.selpcnm)
RsquareAdj(hel.rda.all.selpcnm)

# Now that we have these parsimonious models, NOW WE CAN PERFORM VARIANCE PARTITIONING!  Hurray!
colnames(tree.hel.all.pcnm)
varpart(tree.hel.all.pcnm[,10:16], tree.hel.all.pcnm[,c(42,44:45,52)], tree.hel.all.pcnm[,c(21,34)])
# The variance explained by soil variables and spatial structure clearly overlap. 
# It also appears that there is more independent spatial structure in the trees than independent structuring by soil.
# Are the soil variables significant after accounting for spatial structure, and vice versa?
# Let's see.
hel.rda.all.partpcnm=rda(tree.hel.all.pcnm[,10:16], tree.hel.all.pcnm[,c(21,34)], tree.hel.all.pcnm[,c(42,44:45,52)])
anova(hel.rda.all.partpcnm)
# Yes.

hel.rda.all.partpcnm=rda(tree.hel.all.pcnm[,10:16], tree.hel.all.pcnm[,c(42,44:45,52)], tree.hel.all.pcnm[,c(21,34)])
anova(hel.rda.all.partpcnm)
# Yes.
...
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,10:16], tree.hel.all.pcnm[,42:72])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

#New forward selection code
hel.rda.fulsoil.none = rda(tree.hel.all.pcnm[,10:16]~1)
hel.rda.fulsoil = rda(tree.hel.all.pcnm[,10:16], tree.hel.all.pcnm[,42:72])
...
## Let's store a model using the selected variables only
names(tree.hel.all.pcnm)
hel.rda.all.selpcnm = rda(tree.hel.all.pcnm[,10:16] ~ as.matrix(tree.hel.all.pcnm[,c(44,48,51,67)]))
anova(hel.rda.all.selpcnm)
RsquareAdj(hel.rda.all.selpcnm)
...
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,10:20], tree.hel.all.pcnm[,46:88])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared
# now run step 2, the selection part
hel.rda.fulsoil.none = rda(tree.hel.all.pcnm[,10:20]~1)
hel.rda.fulsoil = rda(tree.hel.all.pcnm[,10:20], tree.hel.all.pcnm[,46:88])

hel.rda.fulsoil.full=rda(tree.hel.all.pcnm[,10:20]~tree.hel.all.pcnm$PCNM1+tree.hel.all.pcnm$PCNM2+tree.hel.all.pcnm$PCNM3+tree.hel.all.pcnm$PCNM4+tree.hel.all.pcnm$PCNM5+tree.hel.all.pcnm$PCNM6+tree.hel.all.pcnm$PCNM7+tree.hel.all.pcnm$PCNM8+tree.hel.all.pcnm$PCNM9+tree.hel.all.pcnm$PCNM10+tree.hel.all.pcnm$PCNM11+tree.hel.all.pcnm$PCNM12+tree.hel.all.pcnm$PCNM13+tree.hel.all.pcnm$PCNM14+tree.hel.all.pcnm$PCNM15+tree.hel.all.pcnm$PCNM16+tree.hel.all.pcnm$PCNM17+tree.hel.all.pcnm$PCNM18+tree.hel.all.pcnm$PCNM19+tree.hel.all.pcnm$PCNM20+tree.hel.all.pcnm$PCNM21+tree.hel.all.pcnm$PCNM22+tree.hel.all.pcnm$PCNM23+tree.hel.all.pcnm$PCNM24+tree.hel.all.pcnm$PCNM25+tree.hel.all.pcnm$PCNM26+tree.hel.all.pcnm$PCNM27+tree.hel.all.pcnm$PCNM28+tree.hel.all.pcnm$PCNM29+tree.hel.all.pcnm$PCNM30+tree.hel.all.pcnm$PCNM31+tree.hel.all.pcnm$PCNM32+tree.hel.all.pcnm$PCNM33+tree.hel.all.pcnm$PCNM34+tree.hel.all.pcnm$PCNM35+tree.hel.all.pcnm$PCNM36+tree.hel.all.pcnm$PCNM37+tree.hel.all.pcnm$PCNM38+tree.hel.all.pcnm$PCNM39+tree.hel.all.pcnm$PCNM40+tree.hel.all.pcnm$PCNM41+tree.hel.all.pcnm$PCNM42+tree.hel.all.pcnm$PCNM43)

tree.sel=ordiR2step(hel.rda.fulsoil.none,scope=formula(hel.rda.fulsoil.full),Pin=0.05,R2scope=TRUE)
tree.sel
# Let's store a model using the selected variables only
names(tree.hel.all.pcnm)
hel.rda.all.selpcnm = rda(tree.hel.all.pcnm[,10:20] ~ as.matrix(tree.hel.all.pcnm[,c(46:52,54)]))
anova(hel.rda.all.selpcnm)
RsquareAdj(hel.rda.all.selpcnm)

# Now that we have these parsimonious models, NOW WE CAN PERFORM VARIANCE PARTITIONING!  Hurray!
colnames(tree.hel.all.pcnm)
varpart(tree.hel.all.pcnm[,10:20], tree.hel.all.pcnm[,c(21,23,27)], tree.hel.all.pcnm[,c(46:52,54)])
# The variance explained by soil variables and spatial structure clearly overlap. 
# It also appears that there is more independent spatial structure in the trees than independent structuring by soil.
# Are the soil variables significant after accounting for spatial structure, and vice versa?
# Let's see.
hel.rda.all.partpcnm=rda(tree.hel.all.pcnm[,10:20], tree.hel.all.pcnm[,c(46:52,54)], tree.hel.all.pcnm[,c(21,23,27)])
anova(hel.rda.all.partpcnm)
# Yes.
hel.rda.all.partsoil=rda(tree.hel.all.pcnm[,10:20], tree.hel.all.pcnm[,c(21,23,27)], tree.hel.all.pcnm[,c(46:52,54)])
anova(hel.rda.all.partsoil)
# Also yes.  But only by p-value; that is really not much variance explained anymore.
...
names(tree.hel.soil.spec)
hel.rda.selsoil = rda(tree.hel.soil.spec[,10:20] ~ as.matrix(tree.hel.soil.spec[,c(21,27)]))
anova(hel.rda.selsoil)
RsquareAdj(hel.rda.selsoil)
plot(hel.rda.selsoil)
# Cool analysis, but what a terrible plot.  Much better to get the scores and plot them in a real graphing program, or do some fancy graphing in R if that is your thing.
...
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,10:20], tree.hel.all.pcnm[,46:70])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared
# now run step 2, the selection part
hel.rda.fulsoil.none = rda(tree.hel.all.pcnm[,10:20]~1)
hel.rda.fulsoil = rda(tree.hel.all.pcnm[,10:20], tree.hel.all.pcnm[,46:70])
...
# Let's store a model using the selected variables only
names(tree.hel.all.pcnm)
hel.rda.all.selpcnm = rda(tree.hel.all.pcnm[,10:20] ~ as.matrix(tree.hel.all.pcnm[,c(46:48,50:51,56)]))
anova(hel.rda.all.selpcnm)
RsquareAdj(hel.rda.all.selpcnm)

# Now that we have these parsimonious models, NOW WE CAN PERFORM VARIANCE PARTITIONING!  Hurray!
colnames(tree.hel.all.pcnm)
varpart(tree.hel.all.pcnm[,10:43], tree.hel.all.pcnm[,c(46:48,50:51,56)], tree.hel.all.pcnm[,c(21,27)])
# The variance explained by soil variables and spatial structure clearly overlap. 
# It also appears that there is more independent spatial structure in the trees than independent structuring by soil.
# Are the soil variables significant after accounting for spatial structure, and vice versa?
# Let's see.
hel.rda.all.partpcnm=rda(tree.hel.all.pcnm[,10:43], tree.hel.all.pcnm[,c(46:48,50:51,56)], tree.hel.all.pcnm[,c(21,27)])
anova(hel.rda.all.partpcnm)
# NO.
hel.rda.all.partsoil=rda(tree.hel.all.pcnm[,10:43], tree.hel.all.pcnm[,c(21,27)], tree.hel.all.pcnm[,c(46:48,50:51,56)])
anova(hel.rda.all.partsoil)
# Also yes.  But only by p-value; that is really not much variance explained anymore.
...
names(tree.hel.soil.spec)
hel.rda.selsoil = rda(tree.hel.soil.spec[,10:20] ~ as.matrix(tree.hel.soil.spec[,c(24,23,27)]))
anova(hel.rda.selsoil)
RsquareAdj(hel.rda.selsoil)
plot(hel.rda.selsoil)
# Cool analysis, but what a terrible plot.  Much better to get the scores and plot them in a real graphing program, or do some fancy graphing in R if that is your thing.
...
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,10:20], tree.hel.all.pcnm[,46:84])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared
# now run step 2, the selection part
hel.rda.fulsoil.none = rda(tree.hel.all.pcnm[,10:20]~1)
hel.rda.fulsoil = rda(tree.hel.all.pcnm[,10:20], tree.hel.all.pcnm[,46:84])
...
# Let's store a model using the selected variables only
names(tree.hel.all.pcnm)
hel.rda.all.selpcnm = rda(tree.hel.all.pcnm[,10:43] ~ as.matrix(tree.hel.all.pcnm[,c(46:47,49,51:52,54)]))
anova(hel.rda.all.selpcnm)
RsquareAdj(hel.rda.all.selpcnm)

# Now that we have these parsimonious models, NOW WE CAN PERFORM VARIANCE PARTITIONING!  Hurray!
colnames(tree.hel.all.pcnm)
varpart(tree.hel.all.pcnm[,10:20], tree.hel.all.pcnm[,c(24,23,27)], tree.hel.all.pcnm[,c(46:47,49,51:52,54)])
# The variance explained by soil variables and spatial structure clearly overlap. 
# It also appears that there is more independent spatial structure in the trees than independent structuring by soil.
# Are the soil variables significant after accounting for spatial structure, and vice versa?
# Let's see.
hel.rda.all.partpcnm=rda(tree.hel.all.pcnm[,10:20], tree.hel.all.pcnm[,c(46:47,49,51:52,54)], tree.hel.all.pcnm[,c(24,23,27)])
anova(hel.rda.all.partpcnm)
# Yes.
hel.rda.all.partsoil=rda(tree.hel.all.pcnm[,10:20], tree.hel.all.pcnm[,c(24,23,27)], tree.hel.all.pcnm[,c(46:47,49,51:52,54)])
anova(hel.rda.all.partsoil)
# Also yes.  But only by p-value; that is really not much variance explained anymore.
...
tree.hel.soil=tree.hel.soil[,c(1:9,11,14,20:22,24,29,32,35,39,41,52:76)]

# Here is the global analysis. Can proceed if significant.  Use adj. R square as additional stopping criterion.
hel.rda.fulsoil = rda(tree.hel.soil[,10:20], tree.hel.soil[,21:45])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared
...
names(tree.hel.all.pcnm)
## Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,10:20], tree.hel.all.pcnm[,46:79])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared
...
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,10:20], tree.hel.all.pcnm[,46:64])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared
...
names(tree.hel.soil.spec)
hel.rda.selsoil = rda(tree.hel.soil.spec[,10:20] ~ as.matrix(tree.hel.soil.spec[,c(23:24,27)]))
anova(hel.rda.selsoil)
RsquareAdj(hel.rda.selsoil)
plot(hel.rda.selsoil)
# Cool analysis, but what a terrible plot.  Much better to get the scores and plot them in a real graphing program, or do some fancy graphing in R if that is your thing.
...
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,10:16], tree.hel.all.pcnm[,37:61])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared
...
tree.hel.soil=tree.hel.soil[,c(1:9,11,13,19:21,23,26,29,47:71)]

# Here is the global analysis. Can proceed if significant.  Use adj. R square as additional stopping criterion.
hel.rda.fulsoil = rda(tree.hel.soil[,10:17], tree.hel.soil[,18:42])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared
...
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,10:17], tree.hel.all.pcnm[,43:78])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared
...
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,10:17], tree.hel.all.pcnm[,43:63])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared
...
# Here is the forward selection command and then constructing a model with selected variables.
hel.rda.fulsoil.none = rda(tree.hel.soil.spec[,10:17]~1)
hel.rda.fulsoil = rda(tree.hel.soil.spec[,10:17], tree.hel.soil.spec[,18:42])

hel.rda.fulsoil.full=rda(tree.hel.soil.spec[,10:17]~tree.hel.soil.spec$Ptot_long+tree.hel.soil.spec$Po_long+tree.hel.soil.spec$pH_long+tree.hel.soil.spec$moist_long +tree.hel.soil.spec$ps1_long+tree.hel.soil.spec$ps2_long+tree.hel.soil.spec$percc_long+tree.hel.soil.spec$cton_long+tree.hel.soil.spec$Ptot_nug+tree.hel.soil.spec$Po_nug+tree.hel.soil.spec$pH_nug+tree.hel.soil.spec$moist_nug+tree.hel.soil.spec$ps1_nug+tree.hel.soil.spec$ps2_nug+tree.hel.soil.spec$percc_nug+tree.hel.soil.spec$pomc_nug+tree.hel.soil.spec$cton_nug+tree.hel.soil.spec$Ptot_short+tree.hel.soil.spec$Po_short+tree.hel.soil.spec$pH_short+tree.hel.soil.spec$moist_short+tree.hel.soil.spec$ps1_short+tree.hel.soil.spec$percc_short+tree.hel.soil.spec$pomc_short+tree.hel.soil.spec$cton_short)

tree.sel=ordiR2step(hel.rda.fulsoil.none,scope=formula(hel.rda.fulsoil.full),Pin=0.05,R2scope=TRUE)
tree.sel

names(tree.hel.soil.spec)
hel.rda.selsoil = rda(tree.hel.soil.spec[,10:17] ~ as.matrix(tree.hel.soil.spec[,c(21,27)]))
anova(hel.rda.selsoil)
RsquareAdj(hel.rda.selsoil)
plot(hel.rda.selsoil)
...
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,10:17], tree.hel.all.pcnm[,43:76])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared
...
#separate out the specialists
tree.hel.soil=tree.hel.soil[,c(1:9,11,14,19:21,25,28,31,34,38,40:42,45:78)]
...
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,10:22], tree.hel.all.pcnm[,57:90])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared
...
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,10:22], tree.hel.all.pcnm[,57:74])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared
...
hel.rda.fulsoil = rda(tree.hel.all.pcnm[,10:22], tree.hel.all.pcnm[,57:74])
...
## Let's store a model using the selected variables only
names(tree.hel.all.pcnm)
hel.rda.all.selpcnm = rda(tree.hel.all.pcnm[,10:28] ~ 1)
anova(hel.rda.all.selpcnm)
RsquareAdj(hel.rda.all.selpcnm)
...
hel.rda.fulsoil.none = rda(tree.hel.soil[,10:22]~1)
hel.rda.fulsoil = rda(tree.hel.soil[,10:22], tree.hel.soil[,33:56])
...
names(tree.hel.soil)
hel.rda.selsoil = rda(tree.hel.soil[,10:22] ~ as.matrix(tree.hel.soil[,c(36,51)]))
anova(hel.rda.selsoil)
RsquareAdj(hel.rda.selsoil)
plot(hel.rda.selsoil)
...
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,10:22], tree.hel.all.pcnm[,57:87])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared
